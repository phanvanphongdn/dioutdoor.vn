...
<div class="shopping-cart-widget-footer<?php echo ( WC()->cart->is_empty() ? ' wd-cart-empty' : '' ); ?>">
	<?php if ( ! WC()->cart->is_empty() ) : ?>
		...
		<?php else : ?>
            <?php if (!is_user_logged_in()) : // vucamp ?>
                <span class="login-mini">Đăng nhập thành viên để nhận ưu đãi</span>
            <?php else : ?>
    <?php //vucamp
    $cart_coupons = WC()->cart->get_coupons();
    if (count($cart_coupons) > 0) :
    $displayed_coupons = array(); // Mảng để theo dõi mã giảm giá đã hiển thị
    foreach ($cart_coupons as $code => $coupon) :
    // Kiểm tra xem mã giảm giá có giảm giá không
    $discount_amount = WC()->cart->get_coupon_discount_amount($code, WC()->cart->display_cart_ex_tax);
    if ($discount_amount > 0 && !in_array($code, $displayed_coupons)) :
    ?>
    <p class="cart-discount coupon-<?php echo esc_attr( sanitize_title( $code ) ); ?>">
        <?php wc_cart_totals_coupon_label($coupon); ?>
        <span class="price-sale-mini"><?php wc_cart_totals_coupon_html($coupon); ?></span>
    </p>
    <?php
    $displayed_coupons[] = $code;
    endif;
    endforeach;
    if (empty($displayed_coupons)) :
        echo '<p class="cart-discount">Bạn chưa đủ điều kiện để giảm giá cho đơn hàng</p>';
    endif;
    else :
        echo '<p class="cart-discount">Bạn chưa đủ điều kiện để giảm giá cho đơn hàng</p>';
    endif;
