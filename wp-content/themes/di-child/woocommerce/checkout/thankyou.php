...
<div class="woocommerce-order<?php echo esc_attr( $wrapper_classes ); ?>">
    ...
    <?php endif; ?><!--vucamp-->
        <?php if( $order->get_payment_method() == 'cod' ){echo do_shortcode('[html_block id="16947"]');}
    elseif ( $order->get_payment_method() == 'bacs' ){echo do_shortcode('[html_block id="16948"]');}
    elseif ( $order->get_payment_method() == 'cheque' ){echo do_shortcode('[html_block id="16950"]');}
        ?><!--end vucamp-->
		<?php if ( woodmart_get_opt( 'thank_you_page_default_content' ) ) : ?>
