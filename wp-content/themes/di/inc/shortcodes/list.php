...
if ( ! function_exists( 'woodmart_list_shortcode' ) ) {
	function woodmart_list_shortcode( $atts ) {
		...
		$list_class .= ' wd-type-' . $list_type;
		$list_class .= ' wd-style-' . $list_style;
		$list_class .= ' text-' . $align;
		...
		if ( 'image' === $list_type && ! empty( $image ) ) {
			...
			$icon_output = '<span class="' . esc_attr( $icon_class ) . '">' . $icon_output . '</span>';
		}
		...
		<ul class="<?php echo esc_attr( $list_class ); ?>" id="<?php echo esc_attr( $list_id ); ?>">
			<?php foreach ( $list_items as $item ) : ?>
				...
				if ( isset( $item['item_type'] ) && 'image' === $item['item_type'] && isset( $item['image_id'] ) ) {
					...
					$item_icon_output = '<span class="' . esc_attr( $icon_class ) . '">' . $item_icon_output . '</span>';
				}
				?>
				<li>
